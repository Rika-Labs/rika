...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: bunx @mariozechner/pi-ai <command> [provider]
...
Examples:
  ...
  bunx @mariozechner/pi-ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'bunx @mariozechner/pi-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'bunx @mariozechner/pi-ai --help' for usage`);
	process.exit(1);
}
